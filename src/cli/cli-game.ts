import { Color, InputOption, Verbosity } from "../shared/enums";
...
export default async function runCli() {
    ...
    const search = new Search(board, moveGenerator, true);
    const moveList: Move[] = [];
    ...
    search.getBestMove(Verbosity.silent, 3 * MS_PER_SECOND);
    ...
    while(true) {
        ...
        if (playerColor !== Color.none && board.sideToMove !== playerColor) {
            [move, score] = search.getBestMove(Verbosity.debug);
