...
export default class SquareAttacks  {
    ...
    constructor(to64: number) {
        let offset = 0;
        while (offset + SQ64_SIZE_BYTES < 32) {
    ...
    public update(piece: Piece, from64: Square, to64: Square, multiplier: number) {
        ...
        if (multiplier === 1) { // adding to bucket
            for (let i = 0; i < bucketSize; i++) {    
                if (((this.value[dWord] >> (bucketOffset + i * SQ64_SIZE_BYTES)) & BIT_MASK_6) === to64) {
                    this.value[dWord] &= (BIT_MASK_6 << (bucketOffset + i * SQ64_SIZE_BYTES)) ^ BIT_MASK_32;
                    this.value[dWord] |= (from64 << (bucketOffset + i * SQ64_SIZE_BYTES));
                    break;
                }
            }
            return;
        ...
        for (let i = 0; i < bucketSize; i++) {    
            if (((this.value[dWord] >> (bucketOffset + i * SQ64_SIZE_BYTES)) & BIT_MASK_6) === from64) {
                const lastPieceSq = ((this.value[dWord] >> (bucketOffset + (bucketSize - 1) * SQ64_SIZE_BYTES)) & BIT_MASK_6);
                this.value[dWord] &= (BIT_MASK_6 << (bucketOffset + i * SQ64_SIZE_BYTES)) ^ BIT_MASK_32;
                this.value[dWord] |= (lastPieceSq << (bucketOffset + i * SQ64_SIZE_BYTES)); // set to last in bucket

                this.value[dWord] &= (BIT_MASK_6 << (bucketOffset + (bucketSize - 1) * SQ64_SIZE_BYTES)) ^ BIT_MASK_32;
                this.value[dWord] |= (to64 << (bucketOffset + (bucketSize - 1) * SQ64_SIZE_BYTES)); // delete last
                break;
            }
        }
    }
    ...
    public *getSmallestAttacker(to64: Square): IterableIterator<Square> {
        for (let bucketIdx = 0; bucketIdx < BUCKET_OFFSET.length; bucketIdx++) { // for each bucket (piece value tier)
            for (let bucket = 0; bucket < BUCKET_SIZE[bucketIdx]; bucket++) { // yield each piece in bucket (square that the piece is on)
                const pieceSq = ((this.value[BUCKET_DWORD[bucketIdx]] >> (BUCKET_OFFSET[bucketIdx] + bucket * SQ64_SIZE_BYTES)) & BIT_MASK_6);
